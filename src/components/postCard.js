    ...
    <article className={`group overflow-clip ${size !== "large" ? 'aspect-w-4  aspect-h-6 md:aspect-h-4' : 'aspect-w-4 aspect-h-6 md:aspect-w-6'} post-card border border-black `} >
      {slug && <Link to={`/read/${slug}`}><div className="flex justify-between flex-col h-full">
        <div className="h-full flex-1 overflow-hidden">
          {featured_media && <div className="aspect-w-16 aspect-h-9 m-1 bg-black"><img className="" src={featured_media} alt={title} /></div>}
          <div className="grid grid-cols-12 h-full border-t border-black group-hover:border-white mx-1">
          {title && <h4 className="mx-4 mt-2 text-xl md:text-3xl col-span-6 border-r border-black group-hover:border-white mr-2 pr-2"  dangerouslySetInnerHTML={{__html: title }} />}
